...
class PurchaseOrderSupplier extends CI_Controller {
    ...
    public function detail($id){
        ...
        $data['detail_po_sup'] = $this->M_PurchaseOrderSupplier->selectHanyaSatuDetail($id_po)->result_array();
    ...
    public function jumlah_minta(){
        $id_permintaan_pembelian = $this->input->post("id_permintaan_pembelian");
        $result = $this->M_PembelianMaterial->selectPermintaanPembeliannya($id_permintaan_pembelian)->result_array();
        echo json_encode($result);
    }
    ...
    public function invoice($status){
        $data['status']=$status;
        $data['inv'] = $this->M_PurchaseOrderSupplier->selectInvoiceInAktif()->result_array();
    ...
    public function invoice_baru(){
        $data['jumlah_inv'] = $this->M_PurchaseOrderSupplier->selectAllInvoiceIn()->num_rows();
        $data['inv'] = $this->M_PurchaseOrderSupplier->selectInvoiceInAktif()->result_array();
        $data['po'] = $this->M_PurchaseOrderSupplier->selectPOSupplierAktif()->result_array();
    ...
    public function get_detail_po(){
        $id_po = array(
            "id_purchase_order_supplier" => $this->input->post("id_po")
        );
        $result = $this->M_PurchaseOrderSupplier->selectSatuDetailPOSupplier($id_po)->result_array();
        echo json_encode($result);
    }
    ...
    public function get_tagihan(){
        ...
        $result = $this->M_PurchaseOrderSupplier->selectSatuPOSupplier($id_po)->result_array();
        echo json_encode($result);
    }

    public function insert_invoice(){
        $data = array (
            "id_invoice_in" => $this->input->post("id_inv"),
            "id_purchase_order_supplier" => $this->input->post("po"),
            "nomor_invoice_in" => $this->input->post("no_inv"),
            "nomor_fp" => $this->input->post("no_fp"),
            "total_harga" => $this->input->post("tagihan"),
            "ppn" => $this->input->post("ppn"),
            "total_bayar" => $this->input->post("total"),
            "tanggal_terima_invoice" => $this->input->post("tgl_inv"),
            "tanggal_pelunasan" => $this->input->post("tgl_lunas"),
            "status_lunas" => "0", //otomatis aktif
        ...
        $this->M_PurchaseOrderSupplier->insertInvoiceIn($data);
        redirect('PurchaseOrderSupplier/invoice/0');
    }

    public function lunas($id){ //edit lunas/belum
        $where = array(
            "id_invoice_in" => $id
        );
        $data = array (
            "tanggal_pelunasan" => date('Y-m-d'),
            "status_lunas" => "1", //otomatis aktif
            "user_edit"=>$_SESSION['id_user'],
            "waktu_edit"=>date('Y-m-d H:i:s')
        );
        $this->M_PurchaseOrderSupplier->editInvoiceIn($data, $where);
        redirect('PurchaseOrderSupplier/invoice/1');
    }

    public function detail_invoice($id){
        $data['id'] = $id;
        $data['invoice'] = $this->M_PurchaseOrderSupplier->selectSatuInvoiceIn($id)->result_array();
		...
		$this->load->view('v_detail_invoice_in', $data);
    }
}