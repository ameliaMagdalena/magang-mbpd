...
class PurchaseOrderCustomer extends CI_Controller {
    ...
    public function detail($id){
        ...
        $data['pengambilann'] = $this->M_PurchaseOrderCustomer->pengambilanMaterial($id_po)->result_array();
        $data['persediaann'] = $this->M_PurchaseOrderCustomer->persediaanLine($id_po)->result_array();
        $data['hargaa'] = $this->M_PurchaseOrderCustomer->hargaMaterial()->result_array();
    ...
    public function insert(){
        ...
        for($x=0; $x<=$row; $x++){
            if($x==0){ //baris pertama
                $jumlah_produk = $this->input->post("jumlah".$x);

                for($z=$row; $z>$x; $z--){ //cek apakah baris dibawah ada yg sama
                    if($this->input->post("produk".$x) == $this->input->post("produk".$z)
                        && $this->input->post("tgl_terima".$x) == $this->input->post("tgl_terima".$z)){ //jika ada yg sama
                        $jumlah_produk = $jumlah_produk + $this->input->post("jumlah".$z); //tambah jumlah produknya
                    }
                }
            ...
            else if($x<$row && $x>0){ //mulai baris kedua
                for($y=0; $y<$x; $y++){ //cek apakah baris diatas ada yg sama
                    if($this->input->post("produk".$x) == $this->input->post("produk".$y)
                        && $this->input->post("tgl_terima".$x) == $this->input->post("tgl_terima".$y)){ //jika ada yg sama
                        $tampung = 1;
                    }
                }
                ...
                if($tampung != 1){
                    ...
                    for($z=$row; $z>$x; $z--){ //cek apakah baris dibawah ada yg sama
                        if($this->input->post("produk".$x) == $this->input->post("produk".$z)
                            && $this->input->post("tgl_terima".$x) == $this->input->post("tgl_terima".$z)){ //jika ada yg sama
                            $jumlah_produk = $jumlah_produk + $this->input->post("jumlah".$z);
                        }
                    }
