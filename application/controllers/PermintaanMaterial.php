...
class PermintaanMaterial extends CI_Controller {
    ...
    public function setuju(){
        ...
        //input id permintaan di material
        if($this->input->post("status") == 2){ //if material sudah tersedia
            $idper = $this->input->post("idnyaa");
            $detaill = $this->M_PerencanaanMaterial->selectSatuDetailPermintaanMaterial($idper)->result_Array();
            $matt = $this->M_Material->selectMaterialTersedia()->result_Array();

            //loop tabel detail permintaan material
            for($x=0; $x<count($detaill); $x++){
                $butuh = $detaill[$x]['needs'];
                $hitung = 0;

                //loop tabel material
                for($y=0; $y<count($matt); $y++){
                    if($matt[$y]['id_sub_jenis_material'] == $detaill[$x]['id_sub_jenis_material'] && $hitung<$butuh){

                        //edit id detail permintaan di tabel material
                        $data2 = array(
                            "id_detail_permintaan_material" => $detaill[$x]['id_detail_permintaan_material'],
                            "user_edit"=>$_SESSION['id_user'],
                        ...
                        $this->M_Material->editMaterial($data2,$where2);
    ...
    //------------------------------------------------------------------------//
    public function ambil($status){
        $data['status'] = $status;
        $data['today'] = $this->M_PengambilanMaterial->selectPengambilanToday()->result_array();
        $data['ambil'] = $this->M_PengambilanMaterial->selectPengambilanDetail()->result_array();
        ...
        $this->load->view('v_pengambilan_produksi', $data);
    }
    
    //------------------------ PERMINTAAN TAMBAHAN --------------------------//
    //-----------------------------------------------------------------------//
    public function tambahan($status){
        $id = $this->input->post('id');

        $data['status'] = $status;
        $data['tambahan'] = $this->M_PerencanaanMaterial->selectPermintaanTambahanAktif()->result_array();
        $data['detail'] = $this->M_PerencanaanMaterial->selectDetailPermintaanMaterialAktif()->result_array();
        $data['material'] = $this->M_PerencanaanMaterial->selectKetersediaanMaterial()->result_array();
		...
		$this->load->view('v_permintaan_material_tambahan', $data);
    }

    public function detail_tambah($id){
        /* $idnya = array(
            "id_permintaan_material" => $id
        ); */
        $data['id_permintaan'] = $id;
        $data['tambahan'] = $this->M_PerencanaanMaterial->selectSatuPermintaanTambahan($id)->result_array();
        $data['detail'] = $this->M_PerencanaanMaterial->selectSatuDetailPermintaanTambahan($id)->result_array();
        $data['material'] = $this->M_PerencanaanMaterial->selectKetersediaanMaterial()->result_array();
    ...
    public function setuju_tambah(){
        $where = array(
            'id_permintaan_tambahan' => $this->input->post("idnyaa")
        );
        ...
        $this->M_PerencanaanMaterial->editPermintaanTambahan($data, $where);
        
        //input id permintaan di material
        $idper = $this->input->post("idnyaa");
        $detaill = $this->M_PermintaanTambahan->selectSatuPermintaanTambahanAktif($idper)->result_Array();
        $matt = $this->M_Material->selectMaterialTersedia()->result_Array();
        //loop tabel detail permintaan material
        for($x=0; $x<count($detaill); $x++){
            $butuh = $detaill[$x]['jumlah_tambah'];
            $hitung = 0;
            //loop tabel material
            for($y=0; $y<count($matt); $y++){
                if($matt[$y]['id_sub_jenis_material'] == $detaill[$x]['id_sub_jenis_material'] && $hitung<$butuh){
        ...
        if ($this->input->post("status") == 1 || $this->input->post("status") == 2){
            redirect('PermintaanMaterial/tambahan/1');
        }
        else if ($this->input->post("status") == 3){
            redirect('PermintaanMaterial/tambahan/2');
        }
        else{
            redirect('PermintaanMaterial/tambahan/3');
        }
    }

    public function ajax_tambah(){
        $id             = $this->input->post('id');
        $data['permat'] = $this->M_PerencanaanMaterial->selectSatuPermintaanTambahan($id)->result_array();

        echo json_encode($data);
    }

    public function proses_perencanaan_tambah($id){
        $data['id_permintaan'] = $id;
        $data['tambahan'] = $this->M_PerencanaanMaterial->selectSatuPermintaanTambahan($id)->result_array();
        $data['detail'] = $this->M_PerencanaanMaterial->selectSatuDetailPermintaanTambahan($id)->result_array();
        $data['material'] = $this->M_PerencanaanMaterial->selectKetersediaanMaterial()->result_array();
